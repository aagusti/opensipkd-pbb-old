...
@view_config(route_name='upload-logo',
             renderer='templates/upload/file.pt',
...
def view_file(request):
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            dir_path = request.static_url('pbb:static/img')
            
            fullpath = os.path.join(dir_path, 'logo.png')
            import pkg_resources
            print pkg_resources.resource_filename('os.path', 'static')
            print '---------------', fullpath
            output_file = open(fullpath, 'wb')
            input_file.seek(0)
            while True:
